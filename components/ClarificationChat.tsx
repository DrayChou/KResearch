...
const ClarificationChat: React.FC<ClarificationChatProps> = ({ history, onAnswerSubmit, isLoading, onSkip }) => {
    ...
    const placeholderText = getVisibleHistory().length > 0 ? t('yourResponse') : t('researchQuery');

    return (
        <div className="w-full animate-fade-in space-y-4">
            <h2 className="text-xl font-bold text-center text-gray-800 dark:text-gray-200">{t('clarifying')}</h2>
            <p className="text-sm text-center text-gray-600 dark:text-gray-400">{t('aiClarifying')}</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                     <LiquidButton type="submit" disabled={isLoading || !userAnswer.trim()} className="w-full">
                        {isLoading ? t('loading') + '...' : t('submitResponse')}
                    </LiquidButton>
